...
def build_features(input_path, output_path):
    logger.info(f"Building pKa features from {input_path}")
    ...
    for idx, row in tqdm(df.iterrows(), total=total_processed, desc="Generating pKa features"):
...
if __name__ == "__main__":
    input_file = os.path.join(project_root, 'data/processed/pka_cleaned.csv')
    output_file = os.path.join(project_root, 'data/processed/pka_features.csv')
