...
def train_uncertainty_model(data_path, model_dir):
    logger.info("Starting pKa Random Forest training (for uncertainty)...")
    ...
    if X.isnull().values.any():
        logger.warning("Found NaNs in features. Filling with 0.")
        X = X.fillna(0)
    ...
    print("\n--- pKa RF Performance (Uncertainty Provider) ---")
    print(f"Test RMSE:  {rmse_test:.4f}, R2: {r2_test:.4f}")
    
    # Save model
    os.makedirs(model_dir, exist_ok=True)
    model_path = os.path.join(model_dir, 'pka_rf.joblib')
